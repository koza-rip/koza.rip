...
async function startup() {
  await titleData();
   ...
   for (i=0; i<ary_TitleDataBandsAndCardTypes.length; i++) {
      ...
      new_CheckBox.value = ary_TitleDataBandsAndCardTypes[i];
      new_CheckBox.title = ary_TitleDataBandsAndCardTypes[i];
      new_CheckBox.id = 'optSelect' + i;
      optSelArr.push('optSelect' + i);
      new_cell.appendChild(new_CheckBox);

      var new_span = cE('span');
      new_span.appendChild(cT(ary_TitleDataBandsAndCardTypes[i]));
      new_span.title = ary_TitleDataBandsAndCardTypes[i];
      new_span.id = i;
      sC(new_span, 'cbox');
